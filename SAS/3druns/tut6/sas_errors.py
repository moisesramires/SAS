...
writer = csv.writer(open('SAS_tut6_used_samples.csv', 'a', newline=''), delimiter = ";")
...
for N in range(10,11):
	...
	for k in range(0,len(true_y)):
		...
		row=[str(k),testing_rssi[k],str(aSAS), str(bF), str(np.linalg.norm(aSAS -bF)), str(SASpredict_u[k]), training_rssi[k],fet_dist[k], used_cluster[k]  ]
		writer.writerow(row)
