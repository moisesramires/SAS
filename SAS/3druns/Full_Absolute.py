...
for i in range(0,len(Plain_Error)):
    ...
    plt.plot(
        [SASNE[i],BSCNE[i], KMEANSNE[i], PNE[i]],
        [SASNT[i],BSCNT[i], KMEANSNT[i], PNT[i]],label=labels[c],linewidth=4)
    c+=1
...
plt.plot(PNE, PNT, 'o', color='red',label="Plain",markersize=15);
