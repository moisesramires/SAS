...
writer = csv.writer(open('sah1.csv', 'a', newline=''), delimiter = ";")
...
for Ts in [-6,-7,-8]:
	for N in range(1,5):
		...
		row=[str(N*-1),str(Ts), str(statistics.median(EuclideanDistanceBSC)), str(sum(EuclideanDistanceBSC) / len(EuclideanDistanceBSC)), str(minimum),  str(np.amax(EuclideanDistanceBSC) ),  str(np.std(EuclideanDistanceBSC)), str(timeBSC), str(timeCI), str(len(clusters)),str(timeCL)]
		writer.writerow(row)
