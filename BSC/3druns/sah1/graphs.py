...
with open('sah1.csv', newline='') as csvfile:
...
for i in cluster_intersection:
	...
	#12
	#5
	x_dict[(int(i[0])*-1)].append(float(i[3].replace('"', '')))
	x2_dict[(int(i[0])*-1)].append((float(i[7].replace('"', ''))+float(i[8].replace('"', '')))/156*1000)
	x3_dict[(int(i[0])*-1)].append(float(i[8].replace('"', ''))/156*1000)
	x4_dict[(int(i[0])*-1)].append(float(i[9].replace('"', '')))
	T.append(int(i[1])*-1)
