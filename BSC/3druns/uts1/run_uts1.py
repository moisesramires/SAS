...
samples = []
dataset = []
with open('../../../SAS_library/datasets/uts1/UTS1_trnrss_no_unDetected_no_bellow_treshold.csv', newline='') as csvfile:
	spamreader = csv.reader(csvfile, delimiter=';', quotechar='|')
	for row in spamreader:
		samples.append(row) # samples RSSI
		

with open('../../../SAS_library/datasets/uts1/UTS1_trncrd_no_unDetected_no_bellow_treshold.csv', newline='') as csvfile:
	spamreader = csv.reader(csvfile, delimiter=';', quotechar='|')
	for row in spamreader:
...
samples2 = []
dataset2 = []
with open('../../../SAS_library/datasets/uts1/UTS1_tstrss_no_unDetected_no_bellow_treshold.csv', newline='') as csvfile:
	spamreader = csv.reader(csvfile, delimiter=';', quotechar='|')
	for row in spamreader:
		samples2.append(row) # samples RSSI
		

with open('../../../SAS_library/datasets/uts1/UTS1_tstcrd_no_unDetected_no_bellow_treshold.csv', newline='') as csvfile:
	spamreader = csv.reader(csvfile, delimiter=';', quotechar='|')
	for row in spamreader:
...
writer = csv.writer(open('uts1.csv', 'a', newline=''), delimiter = ";")
...
for Ts in [-1, -2, -3,-4,-5]:
	for N in range(7,8):
		...
		for i in samples2:
			...
			#samples_in_cluster=BSCv2.cluster_identification2(i,clusters,samples, 6,-73)
			samples_in_cluster=BSCv2.cluster_identification_using_N_APS(i,clusters, N,-94)
			#samples_in_cluster=BSCv2.cluster_identification_repetition(i,clusters, 6,-73)
