...
samples = []
dataset = []
with open('../../../SAS_library/datasets/man1/MAN1_trnrss_no_unDetected_no_bellow_treshold.csv', newline='') as csvfile:
	spamreader = csv.reader(csvfile, delimiter=';', quotechar='|')
	for row in spamreader:
		samples.append(row) # samples RSSI
		

with open('../../../SAS_library/datasets/man1/MAN1_trncrd.csv', newline='') as csvfile:
	spamreader = csv.reader(csvfile, delimiter=',', quotechar='|')
	for row in spamreader:
...
samples2 = []
dataset2 = []
with open('../../../SAS_library/datasets/man1/MAN1_tstrss_no_unDetected_no_bellow_treshold.csv', newline='') as csvfile:
	spamreader = csv.reader(csvfile, delimiter=';', quotechar='|')
	for row in spamreader:
		samples2.append(row) # samples RSSI
		

with open('../../../SAS_library/datasets/man1/MAN1_tstcrd.csv', newline='') as csvfile:
	spamreader = csv.reader(csvfile, delimiter=',', quotechar='|')
	for row in spamreader:
...
for Ts in [-1]:
	for N in range(4,5):
		...
		timeCL=0
		time1 = time.time()
		clusters=BSCv2.clusters=BSCv2.cluster(1, Ts, samples, -105, -10 )
		...
		for i in samples2:
			...
			time1 = time.time()
			samples_in_cluster=BSCv2.cluster_identification_using_N_APS(i,clusters, N, -101)
