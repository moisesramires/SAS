...
with open('man1.csv', newline='') as csvfile:
...
for i in cluster_intersection:
	...
	x2_dict[(int(i[0]))].append((float(i[7].replace('"', ''))+float(i[8].replace('"', '')))/460*1000)
	x3_dict[(int(i[0]))].append(float(i[8].replace('"', ''))/460*1000)
...
for j in N:
	...
	axs[0, 1].scatter(x1, x_dict[j], label = "N="+ str(j), s=50)
	...
	axs[1, 1].scatter(x1, x2_dict[j], label = "N="+ str(j), s=50)
	...
	axs[1, 0].scatter(x1, x3_dict[j], label = "N="+ str(j), s=50)
	...
	axs[0, 0].scatter(x1, x4_dict[j], label = "N="+ str(j), s=50)
